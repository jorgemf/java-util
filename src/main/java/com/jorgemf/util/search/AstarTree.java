...
import com.jorgemf.util.tree.Btree;
import com.jorgemf.util.tree.BtreeVisitor;
...
public class AstarTree implements BtreeVisitor {
    ...
    public AstarTree(Operation[] operations, Heuristic[] heuristics, ResourcesFactory<State> statesFactory) {
        this.operations = operations;
        this.heuristics = heuristics;
        this.visitedStates = new HashSet<String>();
        this.openStateList = new Btree(heuristics.length);
    ...
    public State start(long time, int maximumVisitedStates, State initialState) {
        ...
        if (initialState != null) {
            ...
            currentStates = openStateList.getFirst();
        }

        while (bestHeuristic > 0 && targetTime >= System.currentTimeMillis() && currentVisitedStates < maximumVisitedStates && !openStateList.isEmpty()) {
            currentStates = openStateList.getFirst();

            if (DEBUG) {
                System.out.println("---------- A STAR (get first nodes, one per heuristic if there are enough nodes) ----------");
            }
            ...
            for (heuristicIndex = 0; heuristicIndex < currentStates.length; heuristicIndex++) {
                ...
                if (current != null) {
                    ...
                    for (i = 0; i < heuristicsLength; i++) {
                        if (heuristicsArray[i] < bestHeuristic || (heuristicsArray[i] == bestHeuristic && current.cost < bestState.cost)) {
                            if (bestState != null) {
                    ...
                    for (operatorIndex = 0; operatorIndex < operatorsLength; operatorIndex++) {
                        if (DEBUG) {
                            System.out.println("-------------- Operator: " + operations[operatorIndex].getName());
                        }
                        t1 = System.nanoTime();
                        offsprings.clear();
                        ...
                        for (i = 0; i < offsprings.size(); i++) {
                            ...
                            if (exists) {
                                ...
                                for (i = 0; i < heuristicsLength; i++) {
                                    t1 = System.nanoTime();
                                    offspring.heuristic[i] = heuristics[i].calculateHeuristic(offspring);
                                    t2 = System.nanoTime();
                                    timeHeuristic += t2 - t1;
    ...
    private void printStats() {
        long timeExecuting = System.currentTimeMillis() - this.startAlgorithmTime + this.accumulatedTime;
        ...
        for (int i = 0; i < timeOperators.length; i++) {
            ...
            System.out.println("\t\t time per visit " + (timeOperators[i] * 1.0 / visitedStatesNumber));
            System.out.println("\t\t generated states (per visit): " + this.generatedStatesOperator[i] + " (" + (this.generatedStatesOperator[i] * 1.0 / visitedStatesNumber) + ")");
            System.out.println("\t\t repeated generated states (per visit): " + this.repeatedGeneratedStatesOperator[i] + " (" + (this.repeatedGeneratedStatesOperator[i] * 1.0 / visitedStatesNumber) + ")");
            System.out.println("\t\t collisions: " + this.repeatedGeneratedStatesOperator[i] * 100.0 / this.generatedStatesOperator[i]);
        }
        ...
        System.out.println("Time heuristic per generatedState state: " + heuristicPerGenerateState + " ns");
        for (int i = 0; i < timeHeuristics.length; i++) {
            ...
            System.out.println("\t\t time por generatedState " + (timeHeuristics[i] * 1.0 / generatedStates));
        }
        ...
        long realTreeSize = this.openStateList.getSize();
